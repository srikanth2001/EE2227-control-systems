...
The feedback current amplifier in fig.\ref{fig:Original ckt1}a can
...
configuration shown in Fig.\ref{fig:Original ckt2}b, where $V_G$ is a dc
bias voltage. Replacing Q1 by its small-signal model, use
the analogy of the resulting circuit to that in Fig.\ref{fig:Original ckt1}a to find $I_o$ and $R_{out}$.?
\begin{figure}[!h]
	\begin{center}
...
\begin{table}[]
    ...
    \caption{ parameter's  table for fig.\ref{fig:Original ckt1}a }
    \label{table:ee18btech11023_1}
\end{table}
\begin{table}[]
    \centering
  	\resizebox{\columnwidth}{!}{\input{tables/EE18BTECH11023/ee18btech11023_table2}}
    \caption{parameter's  table for fig.\ref{fig:Original ckt2}b}
...
\\
\solution \\
Refer to the fig. \ref{fig:Original ckt1}a for the feedback current amplifier circuit, in this super common gate transistor is connected between the gate and source terminals of the MOSFET.\\
Replace the op-amp with its equivalent modal and replace the MOSFET with its small signal equivalent circuit.\\
with reference to the fig,\ref{fig:smallckt1}. For ideal op-amp,the input resistance($R_{id}$) is very high (infinite) . And the drain current is approximately equal to source current
\begin{align}
    I_{D} \cong  I_{S}
...
from fig.\ref{fig:smallckt1} we can observe that input voltage is,
\begin{align}
    V_{in} = -R_sI_s
...
\end{align}
and from fig.\ref{fig:smallckt1} the input Resistance is equal to $R_{id}$
\begin{align}
    R_{i} = R_{id}
...
the open loop amplifier circuit ('G' circuit ) fig.\ref{fig:smallckt1}
\begin{figure}[!h]
	\begin{center}
...
For a ideal op-amp the output Resistance is very small, so we can neglect the resistance $r_o$.  from fig.\ref{fig:smallckt1}
\begin{align}
    V_{id} = I_sR_s
...
\end{align}
from the fig.\ref{fig:smallckt1}
\begin{align*}
    since \;\; R_{id} = \infty
\end{align*}
...
(d) Find GH and T . If $\mu$ is large, what is the value of T ?}\\
\solution\\
from eq.\ref{eq_ee18btech11023_11} and from eq.\ref{eq_ee18btech11023_16}
\begin{align*}
    G = \mu g_m R_s\;\; ,  \;\;\; H = 1
\end{align*}
value of GH:
...
\end{align}
From the figure Fig.\ref{fig:smallckt1} 
\begin{align}
    R_o = \frac{1}{g_m}
...
configuration shown in Fig.\ref{fig:Original ckt2}b, where $V_G$ is a dc
bias voltage. Replacing Q1 by its small-signal model, use
the analogy of the resulting circuit to that in Fig.\ref{fig:Original ckt1}a to find Io and Rout.}
\begin{figure}[!h]
	\begin{center}
...
\solution\\
Refer to the fig.\ref{fig:Original ckt2}b for the cascode configuration in which the super CG transistor is used.  the small signal equivalent circuit is shown in 
...
\begin{table}[]
    ...
    \caption{parameter's table for fig.\ref{fig:spice fig}}
...
\begin{figure}[!h]
    ...
    \caption{output of feedback Amplifier}
    \label{fig:output}
\end{figure}
